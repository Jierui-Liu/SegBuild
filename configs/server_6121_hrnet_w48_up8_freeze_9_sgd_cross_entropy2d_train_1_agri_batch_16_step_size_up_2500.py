...
config = dict(
    ...
    pretrain = r"",
    multi_gpu = True,
    device_ids = [0,1,2,3], # 默认第一位作为主卡

    # Solver
    enable_swa = False,
    criterion = dict(type="cross_entropy2d"),
    lr_scheduler = dict(type="CyclicLR",base_lr=1e-6,max_lr=1e-3,step_size_up=2500,mode='triangular2',cycle_momentum=True), # cycle_momentum=False if optimizer==Adam
