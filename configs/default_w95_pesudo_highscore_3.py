...
config = dict(
    ...
    pretrain = r"/home/liujierui/proj/SegBuild_master/output/model/hrnet_w48_ocr_up4_default_w9_pesudo_highscore_40000.pth",         #file/path of the pretrain model
    multi_gpu = True,
    device_ids = [0], # 默认第一位作为主卡

    # Solver
    enable_swa = False,
    criterion = dict(type="CrossEntropy_ocr", num_outputs = 2,balance_weights=[0.4,1.0]),
    lr_scheduler = dict(type="CyclicLR",base_lr=1e-6,max_lr=1e-2,step_size_up=7000,mode='triangular2',cycle_momentum=True), # cycle_momentum=False if optimizer==Adam
